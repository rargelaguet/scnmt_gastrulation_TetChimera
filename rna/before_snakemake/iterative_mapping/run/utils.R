...
mnn.fn <- function(sce.query, sce.atlas, genes = NULL, npcs = 50, k = 15, cosineNorm = TRUE) {
  ...
  block <- c(sce.atlas$sample,sce.query$plate) %>% as.factor
  ...
  if (isTRUE(cosineNorm)) {
    ...
    if (length(unique(sce.query$plate))>1) {
      sce.query <- multiBatchNorm(sce.query, batch=as.factor(sce.query$plate))
    } else {
  ...
  # if (is.null(genes)) {
  #   genes <- getHVGs(sce_all, block=block, assay.type = "cosineNorm")
