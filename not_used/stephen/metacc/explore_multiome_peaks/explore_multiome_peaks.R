...
# optionally remove peaks that come from >1 celltype
marker_peaks.dt <- marker_peaks.dt[N==1]
...
## ------------------------------------------------------------------------------------------------------------------------
to.plot_met[,.N, stage_lineage][order(-rank(N))]
to.plot_met[, unique(celltype)]
...
lineages_sub <- c(
  "E8.5_Blood_progenitors_",
  "E8.5_Erythroid",
  "E8.5_Haematoendothelial_progenitors"
)
...
# split by sorting instead of cell type

p <- ggplot(to.plot_met[], aes(x = markers, y = rate, fill = class)) +
  geom_boxplot(outlier.shape=NA, coef=1) +
  geom_point(position = position_jitterdodge(jitter.width = 0.5), size = 0.1, alpha = 0.2) +
  labs(x="", y="Methylation") +
  facet_wrap(~celltype, scales="fixed") +
