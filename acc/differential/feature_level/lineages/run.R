...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  io$script <- "/Users/ricard/scnmt_gastrulation_TetChimera/acc/differential/feature_level/lineages/diffacc_feature_level.R"
} else if(grepl("ebi",Sys.info()['nodename'])){
  source("/homes/ricard/scnmt_gastrulation_TetChimera/settings.R")
  io$script <- "/homes/ricard/scnmt_gastrulation_TetChimera/acc/differential/feature_level/lineages/diffacc_feature_level.R"
} else {
  stop("Computer not recognised")
}
io$outdir <- paste0(io$basedir,"/acc/results/differential/feature_level/lineages"); dir.create(io$outdir, showWarnings=F, recursive = T)
io$tmpdir <- paste0(io$basedir,"/acc/results/differential/feature_level/lineages/tmp")
