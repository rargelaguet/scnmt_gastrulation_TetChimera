...
if (grepl("ricard",Sys.info()['nodename'])) {
  ...
  source("/Users/ricard/scnmt_gastrulation_TetChimera/acc/differential/utils.R")
} else if(grepl("ebi",Sys.info()['nodename'])){
  source("/homes/ricard/scnmt_gastrulation_TetChimera/settings.R")
  source("/homes/ricard/scnmt_gastrulation_TetChimera/acc/differential/utils.R")
} else {
...
sample_metadata <- sample_metadata %>%
  .[!is.na(pass_accQC) & lineage%in%c(args$groupA,args$groupB)] %>%
  # .[pass_accQC==TRUE & lineage%in%c(args$groupA,args$groupB)] %>%
...
data <- fread(sprintf("%s/%s.tsv.gz",io$acc_data_parsed,args$anno), 
  showProgress=F, header=F, select = c("V1"="factor", "V2"= "character", "V4"="integer", "V5"="integer", "V6"="integer")
) %>% .[V1%in%sample_metadata$id_acc & V5>=opts$min.GpCs] %>% 
...
#   fread(
#     file = sprintf("%s/tmp/%s_%s.gz",io$acc_data_parsed,i,args$anno), 
#     showProgress = FALSE, header = FALSE,
#     select = c("V1"="factor", "V2"= "character", "V4"="integer", "V5"="integer", "V6"="integer"))
# }) %>% rbindlist %>% setnames(c("id_acc","id","Nmet","Ntotal","rate"))
